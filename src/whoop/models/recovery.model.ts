...
export class WhoopRecovery extends Model<WhoopRecovery> {
  @ForeignKey(() => WhoopCycle) @Column({ type: DataType.BIGINT, primaryKey: true }) declare cycle_id: number;
  @ForeignKey(() => WhoopSleep) @Column({ type: DataType.UUID, primaryKey: true }) declare sleep_id: string;
